...
void LvlScene::setItemPlacer(int itemType, unsigned long itemID, int dType)
{
    ...
    case 2: //npcs
    {
        ...
        LvlPlacingItems::npcGfxOffsetX2 = long(-((double) mergedSet.setup.gfx_offset_x));
    ...
    case 3: //water
        ...
        LvlPlacingItems::waterType = int(itemID);
    ...
    case 5: //PlayerPoint
    {
        ...
        LvlPlacingItems::playerID = int(itemID);
        LvlPlacingItems::gridSz = 16;
        ...
        {
        case 1:
            playerPixmap = Themes::Image(Themes::player1);
            break;
        case 2:
            playerPixmap = Themes::Image(Themes::player2);
            break;
        default:
            playerPixmap = Themes::Image(Themes::player_point);
            break;
        }

        PlayerPoint x = FileFormats::CreateLvlPlayerPoint(static_cast<unsigned int>(itemID + 1));
...
void LvlScene::setRectDrawer()
{
    ...
    {
        QPixmap oneCell = QPixmap(int(LvlPlacingItems::itemW), int(LvlPlacingItems::itemH));
        oneCell.fill(QColor(0xFF, 0xFF, 0x00, 128));
        QPainter p(&oneCell);
        p.setBrush(Qt::NoBrush);
        p.setPen(QPen(Qt::yellow, 2, Qt::SolidLine));
        p.drawRect(0, 0, int(LvlPlacingItems::itemW), int(LvlPlacingItems::itemH));
